...
parser.add_argument('--model', choices=['lstm', 'resnet', 'resnet_pca', 'wideresnet', 'lcnn_9', 'lcnn_29', 'lcnn_9_pca', 'lcnn_29_pca', 'lcnn_9_prodspec', 'lcnn_9_icqspec', 'lcnn_9_CC', 'lcnn_29_CC', 'resnet_CC', 'TDNN', 'TDNN_multipool', 'TDNN_ablation', 'TDNN_cat', 'TDNN_LSTM', 'FTDNN', 'Linear', 'mobilenet', 'densenet', 'VGG', 'all'], default='resnet', help='Model arch')
...
if args.model == 'TDNN_ablation' or args.model == 'all':
	batch = torch.rand(3, 1, 90, 300)
	model = model_.TDNN_ablation()
	mu = model.forward(batch)
	print('TDNN_ablation', mu.size())
if args.model == 'TDNN_cat' or args.model == 'all':
	batch = torch.rand(3, 1, 90, 300)
	model = model_.TDNN_cat()
	mu = model.forward(batch)
	print('TDNN_ablation', mu.size())
